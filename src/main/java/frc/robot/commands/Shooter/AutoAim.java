...
public class AutoAim extends Command {
  ...
  private PIDController rotationPID = new PIDController(2, 0.5, 0.1);
  ...
  public void execute() {
    ...
    Logger.recordOutput("Shooting/angle", localTagPos.getTranslation().getAngle());
    
    m_swerveSubsystem.drive(new Translation2d(0, 0), rotationPID.calculate(localTagPos.getTranslation().getAngle().getRadians()), false);


    double dist = localTagPos.getTranslation().getNorm();

    System.out.println(dist + " at: ");
    
    double shooterPos = TrajectoryCalculator.getAngle(dist);
    double shooterVel = TrajectoryCalculator.getVelocity(dist);
    
    System.out.println(dist + " at:2222 ");
    
    Logger.recordOutput("Shooting/Target Pos", targetPose);
    Logger.recordOutput("Shooting/Turret Pos", globalRobotPose);
